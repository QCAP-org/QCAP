...
- Each participant generates a random Bitcoin private key and derives their corresponding public keys on both `secp192r1` and `secp256k1` curves.
- Outputs are saved in `../outputs/participant/participant_id/keys`.
- Each participant generates the proofs for the DLEQ, DELQAG, and rangeproofs for the generated keys and stores them in `../outputs/participant/participant_id/proofs`.
...
### 4. Canary Commitment (c4_CanaryCommitment.py)
- The coordinator creates the tweaked public key for the taproot address from the aggregated public key on `secp256k1`. 
- It then creates the `OP-RETURN` for the funding transaction being the CID of the IPFS file. 
- It initiates the crowdfunding process by publishing the transaction sending some initial funds to the created taproot address from the aggregated public key. 
...
- **Directory Structure**:
  ...
  - `outputs/participant/participant_#id/proofs/proof_#publickey.json`: DLEQ and DLEQAG proofs for the key pairs on `secp192r1` and `secp256k1`.
